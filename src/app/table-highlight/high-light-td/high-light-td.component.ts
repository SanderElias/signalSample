...
/**
 * Highlights matching text in a table cell using <mark>, based on a signal from the parent tbody.
 */
@Component({
  selector: 'td',
  imports: [],
...
export class HighLightTDComponent {
  ...
  shadowCell = inject(DOCUMENT).createElement('span'); // create a new span to use for the highlighted version of the content.
  ...
  /**
   * Highlights matching text in the shadow-cell using <mark>; returns true if highlight applied.
   */
  ...
  constructor() {
    ...
    this.elm.appendChild(this.shadowCell); // add the empty span, so its ready to go.
